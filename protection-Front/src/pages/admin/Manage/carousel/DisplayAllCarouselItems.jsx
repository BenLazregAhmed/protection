...
function DisplayAllCarouselItems() {
  ...
  const handleAdd=()=>{
        fetch(base_url+'/set',{
          ...
          body: JSON.stringify({titre:titre,text:text,image:image,path:path})
        })
        setAdd(false)
        fetchUsers()
      }
...
const handleReload = ()=>{
  ...
  const handleUpdate=()=>{
    fetch(base_url+'/update',{
      method:'POST',
      headers:{'content-type':'application/json'},
      body: JSON.stringify({id:id,titre:titre,text:text,image:image,path:path})
    })
    setUpdate(false)
    fetchUsers()
  } 
      ...
      <>
      <header>
        <NavBar title={'Carousel'} reload={handleReload}></NavBar>
      </header>
      <div>
          ...
          (add || update) &&
           <tr>
            ...
            <td><input type='text' placeholder='titre' value={titre} onChange={e=>{setTitre(e.target.value)}}></input></td>
             <td><input type='text' placeholder='description' value={text} onChange={e=>{setText(e.target.value)}}></input></td>
            <td><input type='text' placeholder='image location' value={image} onChange={e=>{setImage(e.target.value)}}></input></td>
            <td><input type='text' placeholder='URL' value={path} onChange={e=>{setPath(e.target.value)}}></input></td>
...
export default DisplayAllCarouselItems