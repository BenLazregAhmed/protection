...
function DisplayAllParcs() {
  ...
  const handleAdd=()=>{
        fetch(base_url+'/set',{
          ...
          body: JSON.stringify({nom:nom,image:image,url:url})
        })
        setAdd(false)
        fetchUsers()
      }
...
const handleReload = ()=>{
  ...
  const handleUpdate=()=>{
    fetch(base_url+'/update',{
      method:'POST',
      headers:{'content-type':'application/json'},
      body: JSON.stringify({id:id,nom:nom,image:image,url:url})
    })
    setUpdate(false)
  } 
      return (
      <>
      <header>
        <NavBar title={'Parcs'} reload={handleReload}></NavBar>
      </header>
      <div>
...
export default DisplayAllParcs